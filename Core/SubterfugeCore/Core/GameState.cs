...
namespace SubterfugeCore.Core
{
    ...
    public class GameState
    {
        ...
        public List<ICombatable> GameObjectsInRange(float range,RftVector pos)
        {
            var gameObjects = GetAllGameObjects();
            List<ICombatable> gameObjectsinRange = new List<ICombatable>();


            foreach (ICombatable c in gameObjects)
            {
                if (Vector2.Distance(c.GetCurrentPosition(GetCurrentTick()).ToVector2(), pos.ToVector2()) < range)
                    {
                    gameObjectsinRange.Add(c);
                }
            }
